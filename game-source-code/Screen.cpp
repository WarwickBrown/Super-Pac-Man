...
// Function to display the end game screen with "Game Over!" message
bool Screen::endGame() {
    // Start drawing the Game Over screen
    window.BeginDrawing();
    window.ClearBackground(BLACK);  // Clear screen with black background

    // Draw "Game Over!" text in the center of the screen in red
    DrawText("Game Over!", 
             window.GetWidth() / 2 - MeasureText("Game Over!", 60) / 2,  // Center the text horizontally
             window.GetHeight() / 3, 60, RED);  // Position text at 1/3rd the window height

    window.EndDrawing();  // Finish drawing the Game Over screen

    // Display the Game Over screen for ~3 seconds or until player presses ENTER
    for (int i = 0; i < 1800; i++) {  // Loop for approximately 3 seconds at 60 FPS
        window.BeginDrawing();
        window.ClearBackground(BLACK);  // Clear background each frame
        DrawText("Game Over!", 
    ...
    // Close the game window after Game Over screen
    CloseWindow();
